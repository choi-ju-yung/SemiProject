...
/**
 * Servlet implementation class EntireMaxPrice
 */
@WebServlet("/entiremaxprice")
public class EntireMaxPriceServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EntireMaxPriceServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<ProductCategoryTimeList> productlist = new ProductCategoryListService().EntireMaxPrice(cPage, numPerpage);
		
		
		request.setAttribute("productlist", productlist);
		request.getRequestDispatcher("/views/productcategorypage/entiremaxprice.jsp").forward(request, response);
