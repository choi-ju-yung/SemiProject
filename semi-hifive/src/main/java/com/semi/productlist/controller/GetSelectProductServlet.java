...
public class GetSelectProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String pageBar = "";
		int totalData = new ProductCategoryListService().GetProductConditionCount(conditionList);
		...
		List<ProductCategoryTimeList> getselectproduct = new ProductCategoryListService().GetProductCondition(conditionList, numPerpage, cPage);
		
		request.setAttribute("getselectproduct", getselectproduct);
		request.getRequestDispatcher("/views/getselectproduct.jsp").forward(request, response);
	}
