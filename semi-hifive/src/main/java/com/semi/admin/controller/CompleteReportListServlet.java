...
@WebServlet("/completeReportList.do")
public class CompleteReportListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       

    public CompleteReportListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//페이징 처리하기
				...
				List<Report> reportList = new AdminService().completeSelectReportList(cPage,numPerpage);
				...
				int totalData = new AdminService().completeSelectReportCount();
				...
				// 3) 출력할 화면을 선택(이동)
				request.getRequestDispatcher("/views/admin/completReportListAdmin.jsp").forward(request, response);
	}
